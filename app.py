...
def addWaterHistorique():
    if request.method == "POST":
        ...
        if calories > 0:
            make_query(f'UPDATE WELL_BEING SET calories = calories + {calories} WHERE id_user="{id_user}" and date="{date}";',
                       True)
        ...
        if (len(get_user_water_waterIsEmpty(id_user,date)) > 0 and len(get_user_water_waterIsNotEmpty(id_user,date))==0) :
            ...
            make_query(f'INSERT INTO WELL_BEING (id_user,water,date,calories) VALUES("{id_user}","{water}","{date}", 0)', True)
